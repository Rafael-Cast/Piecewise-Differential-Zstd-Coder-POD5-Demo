...
if __name__ == "__main__":
    ...
    input_paths = scan_dir('/data/pgnanoraw/data_demo/DATABASE_POD5/R9_back')
    ...
    with mp.Pool(njobs) as pool:
        for chunk in tqdm(chunks, total=total_number_of_chunks, leave=True, desc='overall'):
            ...
            vbz_decompression_results = list(pool.imap(run_vbz_decompression_test,tqdm(chunk[0], total=chunk_size, desc='VBZ_DESC'))) 
            vbz_compression_results = list(pool.imap(run_vbz_compression_test, tqdm(chunk[1], total=chunk_size, desc='VBZ_COMP')))
            our_compression_results = list(pool.imap(run_our_compression_test, tqdm(chunk[3], total=chunk_size, desc='OUR_COMP')))
            our_decompression_results = list(pool.imap(run_our_decompression_test, tqdm(chunk[2], total=chunk_size, desc='OUR_DESC')))
