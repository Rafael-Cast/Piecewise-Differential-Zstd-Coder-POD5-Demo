...
public:
    ...
    Status set_bam_file(const std::string & bam_file_path, const std::string & bam_index_file_path);
    Status build_bam_idx();
    Status load_levels(const std::string & levels_path);
private:
    SignalTableSchemaDescription m_field_locations;
    arrow::MemoryPool * m_pool;
