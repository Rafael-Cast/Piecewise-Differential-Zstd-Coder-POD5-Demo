...
public:
    ...
    {
        PoreDictionaryIndex res;
        ARROW_ASSIGN_OR_RAISE(res, m_read_table_dict_writers.pore_writer->add(pore_type_data));
        ARROW_RETURN_NOT_OK(m_signal_table_writer->add_pore_type(res, pore_type_data));
        return res;
    }
        ...
        gsl::span<std::int16_t const> const & signal)
    {
        ...
        ARROW_ASSIGN_OR_RAISE(
            std::vector<std::uint64_t> signal_rows, add_signal(read_data, signal));
...
// FIXME: This should work independent of the metadata, right?
    pod5::Status add_complete_read(
        ReadData const & read_data,
...
// TODO: this is the main entry point. FIXME: Other entry points exist
// TODO: duplicated parameters: read_id and read_data => Can factor out to avoid overhead an make changes less impactful => This might affect the second add_signal method which I pressume is used for compressed signals
        ...
        gsl::span<std::int16_t const> const & signal)
    {
        ...
        for (std::size_t chunk_start = 0; chunk_start < signal.size();
            ...
            bool is_last_batch = chunk_start + chunk_size >= signal.size(); 
            ARROW_ASSIGN_OR_RAISE(
                auto row_index, m_signal_table_writer->add_signal(read_data, chunk_span, is_last_batch));
...
pod5::Result<std::vector<SignalTableRowIndex>> FileWriter::add_signal(
    ...
    gsl::span<std::int16_t const> const & signal)
{
    return m_impl->add_signal(read_data, signal);
}
