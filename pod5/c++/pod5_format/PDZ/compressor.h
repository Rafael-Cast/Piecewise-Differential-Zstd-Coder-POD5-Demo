...
public:
    ...
    void reset();
    size_t compress(pod5::ReadData const & read_data, std::size_t sample_count, int16_t const * samples, uint8_t* const & dest, pdz::PDZWriterState & state);
    static inline std::size_t compressed_signal_max_size(std::size_t sample_count, pod5::ReadData read_data) { return sample_count*sizeof(int16_t) + header_size + pdz::overflow_buffer_bytes; }
...
private:
    ...
    void compress_metadata(pdz::Metadata const & metadata, uint8_t* const & dest);
    void compress_header(pdz::Header const & header, uint8_t* const & dest);
