...
if __name__ == "__main__":
    ...
    import random
    for iteration_index in tqdm(range(N), total=N, leave=True, desc='Iteration'):
        chunk_number = 0
        this_iteration_chunks = deepcopy(chunks)
        rnd = random.Random(iteration_index)
        rnd.shuffle(this_iteration_chunks)
        for chunk in tqdm(this_iteration_chunks, total=total_number_of_chunks, leave=False, desc='Single iteration'):
            ...
            csv_dump_path = f"{test_out_path}/chunk_{chunk_number}_iteration_{iteration_index}.csv"
            pd.DataFrame(
                {
                    'path': list(map(lambda x: x.original_path, chunk[0])),
